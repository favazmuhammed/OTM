...
from dataset.image_folder_dataset import CustomImageFolderDataset, PoseDataset
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, **kwargs):
        ...
        concat_mem_file_name = os.path.join(self.data_root, self.test_data_path, 'concat_validation_memfile')
        self.concat_mem_file_name = concat_mem_file_name


    def prepare_data(self):
        # call this once to convert val_data to memfile for saving memory
        if not os.path.isdir(os.path.join(self.data_root, self.test_data_path, 'agedb_30', 'memfile')):
            print('making validation data memfile')
            evaluate_utils.get_val_data(os.path.join(self.data_root, self.test_data_path))

        self.valid_df = pd.read_csv(os.path.join(self.data_root, self.val_data_path, 'validation_data1.csv'))

        if 'otm' in self.head:
            self.train_df = pd.read_csv(os.path.join(self.data_root, './data/webface4m_subset_posedata.csv'))
        ...
        if not os.path.isfile(self.concat_mem_file_name):
            ...
            for key in ['agedb_30', 'cfp_fp', 'lfw', 'cplfw', 'calfw']:
                np_array, issame = evaluate_utils.get_val_pair(path=os.path.join(self.data_root, self.test_data_path),
                                                               name=key,
                                                               use_memfile=False)
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            ...
            self.val_dataset = val_dataset(self.valid_df, self.data_root, self.val_data_path)

        # Assign Test split(s) for use in Dataloaders
        if stage == 'test' or stage is None:
            self.test_dataset = test_dataset(self.data_root, self.test_data_path, self.concat_mem_file_name)
...
def train_dataset(data_root, train_data_path,
    ...
    if use_mxrecord:
        ...
        train_dataset = PoseDataset(train_df = train_df,
                                        root = train_dir,
                                        transform=train_transform,
                                        head = head,
                                        low_res_augmentation_prob=low_res_augmentation_prob,
...
def val_dataset(df, data_root, val_data_path, swap_color_channel=None):
    ...
    # valid_dir = os.path.join(data_root, val_data_path)
    valid_dir = os.path.join(data_root, val_data_path, 'probe')

    valid_dataset = ClassroomValidationDataset(valid_data_df=df, 
