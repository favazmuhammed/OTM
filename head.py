...
def build_head(head_type,
    ...
    if head_type == 'adaface':
    ...
    elif head_type == 'poseface':
        head = PoseFace(embedding_size=embedding_size,
                       classnum=class_num,
                       m=m,
                       h=h,
                       s=s,
    ...
    elif head_type == 'poseface2':
        head = PoseFace2(embedding_size=embedding_size,
                       classnum=class_num,
                       m=m,
                       h=h,
                       s=s,
        ...
        head = OTM(embedding_size=embedding_size,
                       classnum=class_num,
                       m=m,
                       h=h,
                       s=s,
    ...
    elif head_type == 'poseadaface':
        head = PoseAdaFace(embedding_size=embedding_size,
                       classnum=class_num,
                       m=m,
                       h=h,
                       s=s,
    ...
    elif head_type == 'posecoseface':
        head = PoseCoseFace(embedding_size=embedding_size,
                       classnum=class_num,
                       m=m,
                       h=h,
                       s=s,
    ...
    elif head_type == 'posearcface':
        head = PoseArcFace(embedding_size=embedding_size,
                       classnum=class_num,
                       m=m,
                       h=h,
                       s=s,
...
class PoseFace(Module):
    def __init__(self, embedding_size=512, classnum=70722, m=0.4, h=0.333, s=64, z_alpha=0.5, t_alpha=0.01) -> None:
        super().__init__()
        self.m = m
        self.h = h
        self.s = s
        ...
        print('\nPoseFace with the following property')
        print('m', self.m)
        print('h', self.h)
        print('s', self.s)
        print('z_alpha', self.z_alpha)
        print('t_alpha', self.t_alpha)
    ...
    def forward(self, feats: Tensor, norms: Tensor, label: Tensor, z_pose:Tensor) -> Tensor:
        ...
        # print(margin_scaler_pose.shape)
        margin_scaler = self.z_alpha * margin_scaler_quality + (1-self.z_alpha) * margin_scaler_pose
        # print(margin_scaler.shape)
...
class PoseFace2(nn.Module):
    def __init__(self, embedding_size=512, classnum=70722, m=0.4, delta=0.2, h=0.333, s=64, t_alpha=0.01, eps = 1e-3) -> None:
        super().__init__()
        self.m = m
        self.delta = delta
        self.h = h
    ...
    def forward(self, feats: Tensor, norms: Tensor, label: Tensor, z_pose:Tensor) -> Tensor:
        ...
        # update the margin
        # print(z_pose.shape)
        m = torch.add(z_pose.reshape(-1,1) * self.delta, self.m)
        # print(m.shape)
