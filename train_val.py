...
class Trainer(LightningModule):
    def __init__(self, **kwargs):
        ...
        self.head = head.build_head(head_type=self.hparams.head,
                                     ...
                                     s=self.hparams.s,
                                     include_pose=self.hparams.include_pose,
                                     include_quality=self.hparams.include_pose,
                                     )
    ...
    def forward(self, images, labels, z_pose=None):
        embeddings, norms = self.model(images)
        cos_thetas = self.head(embeddings, norms, labels, z_pose)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if 'otm' in self.hparams.head:
            images, z_pose, labels = batch
            cos_thetas, norms, embeddings, labels = self.forward(images, labels, z_pose)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        embeddings1, norms1 = self.model(X1)
        embeddings2, norms2 = self.model(X2)

        return {'embeddings1':embeddings1, 
    ...
    def validation_epoch_end(self, outputs):
        ...
        # print(len(outputs), outputs[0]['embeddings1'].shape, len(outputs[0][0]))
        
        embeddings1 = torch.cat([outs['embeddings1'] for outs in outputs], axis=0).to('cpu').numpy()
        embeddings2 = torch.cat([outs['embeddings2'] for outs in outputs], axis=0).to('cpu').numpy()
        issame = torch.cat([outs['labels'] for outs in outputs], axis=0).to('cpu').numpy()


        tpr, fpr, accuracy, best_thresholds = evaluate_utils.evaluate_classroom(embeddings1, embeddings2, issame, nrof_folds=10)
        # tpr, fpr, accuracy, best_thresholds = evaluate_utils.evaluate_classroom_cosine(embeddings1, embeddings2, issame, nrof_folds=10)
        acc, best_threshold = accuracy.mean(), best_thresholds.mean()
        
        val_logs = {}
        num_val_samples = len(embeddings1)
        val_logs[f'val_acc'] = acc
        val_logs[f'classroom_best_threshold'] = best_threshold
        val_logs[f'num_classroom_samples'] = num_val_samples
    ...
    def configure_optimizers(self):
        ...
        # scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=3, threshold=0.01)

        return [optimizer],[scheduler] #{'optimizer': optimizer,'scheduler': scheduler,'monitor': 'train_loss'}
